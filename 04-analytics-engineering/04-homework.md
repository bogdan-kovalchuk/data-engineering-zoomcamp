...
This homework uses the dbt project in `04-analytics-engineering/taxi_rides_ny/` to transform NYC taxi data and answer questions by querying the built models.
...
> **Note:** By default, dbt uses the `dev` target. Use `--target prod` to build models in the production dataset required for the homework queries below.

After a successful build, the warehouse contains models such as `fct_trips`, `dim_zones`, and `fct_monthly_zone_revenue`.
